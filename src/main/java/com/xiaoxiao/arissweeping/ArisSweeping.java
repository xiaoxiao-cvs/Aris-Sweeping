...
public class ArisSweeping extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            LoggerUtil.info("配置文件加载和验证完成");
        } catch (Exception e) {
            LoggerUtil.severe("配置文件验证失败: " + e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            ...
            cleanupHandler.init();
            LoggerUtil.info("Entity cleanup handler initialized successfully!");
        } catch (Exception e) {
            LoggerUtil.severe("Failed to initialize Entity Cleanup Handler: " + e.getMessage());
            e.printStackTrace();
            // 创建一个最小功能的处理器以避免完全失败
            cleanupHandler = null;
        }
        ...
        try {
            ...
            LoggerUtil.info("Livestock density handler initialized successfully!");
        } catch (Exception e) {
            LoggerUtil.severe("Failed to initialize Livestock Density Handler: " + e.getMessage());
            e.printStackTrace();
        ...
        try {
            ...
            LoggerUtil.info("Commands registered successfully!");
        } catch (Exception e) {
            LoggerUtil.severe("Failed to register commands: " + e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            if (cleanupHandler != null) {
                ...
                LoggerUtil.info("Event listeners registered successfully!");
            } else {
                LoggerUtil.warning("CleanupHandler is null, events not registered. Plugin will run with limited functionality.");
            }
        } catch (Exception e) {
            LoggerUtil.severe("Failed to register event listeners: " + e.getMessage());
            e.printStackTrace();
        }
        
        LoggerUtil.info("Aris Sweeping plugin enabled!");
    }
    ...
    public void onDisable() {
        ...
        try {
            if (cleanupHandler != null) {
                ...
                LoggerUtil.info("Entity cleanup handler shutdown successfully!");
            }
        } catch (Exception e) {
            LoggerUtil.severe("Error shutting down cleanup handler: " + e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            if (livestockHandler != null) {
                ...
                LoggerUtil.info("Livestock density handler shutdown successfully!");
            }
        } catch (Exception e) {
            LoggerUtil.severe("Error shutting down livestock handler: " + e.getMessage());
            e.printStackTrace();
        }
        
        LoggerUtil.info("Aris Sweeping plugin disabled!");
    }
